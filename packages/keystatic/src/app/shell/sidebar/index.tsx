  ...
  return (
    <VStack
      ...
      UNSAFE_className={css({
        boxShadow: `1px 0 0 ${tokenSchema.color.border.muted}`,
      })}
    >
      <SidebarHeader />
      <SidebarGitActions />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text color="inherit" weight="bold" size="medium" truncate>
          {brandName}
        </Text>
      </HStack>
...
function SidebarGitActions() {
  ...
  if (isLocalConfig(config) || config.storage.kind === 'github') {
    return null;
  }
  return (
...
function NavItemOrGroup({ itemOrGroup }: { itemOrGroup: ItemOrGroup }) {
  ...
  const current = isCurrent(itemOrGroup.href);
  ...
  return (
    <NavItem
      ...
      UNSAFE_className={css({
        ...
        marginInline: tokenSchema.size.space.medium,
    ...
    >
      <Text truncate title={itemOrGroup.label} weight={current ? 'semibold' : 'regular'}>
        {itemOrGroup.label}
      </Text>
      {changeElement}
    </NavItem>
