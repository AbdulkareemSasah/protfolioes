...
import { css, useMediaQuery, tokenSchema, transition } from '@keystar/ui/style';
      ...
      <Flex alignItems="center" gap="regular">
        ...
        <ClearSlots>
          <Flex direction="column" gap="small">
            <Text size="small" weight="bold" color="neutralEmphasis">
              {name}
            </Text>
            {name === login ? null : (
...
function useUserData(): UserData | 'unauthorized' | undefined {
  ...
  const [customEmail] = useState(() => {
    if (
      ...
      config.storage.kind === 'github' &&
      config.storage.credentials?.kind === 'custom'
    ) {
      return localStorage.getItem('keystatic-user-email');
    }
    return null;
  });
  ...
  if (isGitHubConfig(config) && user) {
    return {
      avatarUrl: customEmail ? undefined : user.avatarUrl,
      login: customEmail || user.login,
      name: customEmail || user.name || user.login,
    };
  }
